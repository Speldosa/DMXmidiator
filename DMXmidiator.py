                                ...
                                if(Layer2.Program[0][1] == 2): # Sub program 2. Sweep from the sides to the middle.
                                    for Count in range(int(len(Layer1.Array_of_Layer1_objects)/2)):
                                        ...
                                        Layer1.Array_of_Layer1_objects[Count] = Layer1_light_object(
                                            ...
                                            Brightness = Signal(ADSR(After_attack_amplitude=0, After_decay_amplitude=(CC_to_ratio(Layer2.Parameters[2][0]) - (CC_to_ratio(Layer2.Parameters[2][0]) - CC_to_ratio(Layer2.Parameters[6][0])) * (Tmp * 2)) * 0.5, Attack=Tmp * (1-CC_to_ratio(Layer2.Parameters[3][0])) - 0.5, Decay=0, Sustain=1-CC_to_ratio(Layer2.Parameters[3][0]), Release=0), LFO(Waveform = "Sine", Amplitude = 1, Repeat = True, Rate = CC_to_ratio(Layer2.Parameters[3][0]), Phase = -0.25 - Tmp))
                                        )
